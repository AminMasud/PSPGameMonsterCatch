...
/* Frozen Phase 14-30 layout, before map discovery was saved. */
typedef struct {
    uint32_t magic,version;
    int map_id,tile_x,tile_y,facing;
    uint32_t encounter_random;
    ...
    SaveParty party;
    int item_quantities[ITEM_COUNT],embermarks;
    uint32_t npc_defeated,progression_flags;
} Phase3Payload;
...
typedef char phase3_size_must_remain_2512[(sizeof(Phase3Payload)==2512)?1:-1];
typedef char current_size_must_remain_2516[(sizeof(SavePayload)==2516)?1:-1];
...
int save_data_decode(const void *bytes,size_t size,SavePayload *payload)
{
    ...
    if(header[1]==3) {
        Phase3Payload old;SavePayload next={0};
        ...
        next.party=old.party;next.embermarks=old.embermarks;next.npc_defeated=old.npc_defeated;
        next.progression_flags=old.progression_flags;next.discovered_maps=1u<<old.map_id;
        for(int i=0;i<ITEM_COUNT;++i) next.item_quantities[i]=old.item_quantities[i];
        *payload=next;return 1;
    }
    if(header[1]==2) {
        Phase2Payload old;
        SavePayload next={0};
